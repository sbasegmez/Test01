...
var invTools={
	...
	TYPE_RATING: 0,
	TYPE_COMMENTS: 1,
	TYPE_ADDTOFAVORITES: 2,
	TYPE_FAVORITESLIST: 3,

	TYPENAMES: ["Rating", "Comments", "AddToFavorites", "MyFavorites"],
	...
	defaults: {
		...
		adminRole: "[Admin]",   //This role will be author for all interaction documents created. 
		starCount: 5,
		commentDatePattern: "('at' HH:mm 'on' dd.MM.yyyy)",
		commentDefaultRowCount : 10		
	},
	
	// These views will be checked at start. If any of them are missing, we will stop...
	...
	checkConfig: function(config, type) {
		try {
			ccId=config.ccId || "";
			...
			if((interactionDB=="") || ((db=session.getDatabase(database.getServer(), interactionDB, false))==null)) {
				viewScope.put("xInvolveStatus"+ccId, false);
				invTools._errHandler(ccId, "Interaction database ("+interactionDB+") for "+invTools.TYPENAMES[type]+" cannot be opened!", false);
				return false;
			}
			...
			for(i=0; i<invTools.mustHaveViews.length; i++) {
				if(db.getView(invTools.mustHaveViews[i])==null) {
					viewScope.put("xInvolveStatus"+ccId, false);
					invTools._errHandler(ccId, "Interaction database ("+interactionDB+") for "+invTools.TYPENAMES[type]+" has no necessarry views!", false);
					return false;
				}
			}
				
			if(type!=invTools.TYPE_FAVORITESLIST && iId=="") {
				viewScope.put("xInvolveStatus"+ccId, false);
				invTools._errHandler(ccId, "Unable to identify the interaction document!", false);
				return false;
			}

			if(type==invTools.TYPE_RATING && !invTools.calculateTotals(config)) {
				viewScope.put("xInvolveStatus"+ccId, false);
				return false;
			}
			...
			return true;
				...
				viewScope.put("xInvolveStatus"+ccId, false);
				invTools._errHandler(ccId, "Unable to check "+invTools.TYPENAMES[type]+" ("+e+")!", false);
				return false; 						
		}
	},
	...
	calculateTotals: function(config) {
		try {
			...
			ratings=@DbLookup([database.getServer(),interactionDB], "RateCalc", iId, 2, "[FailSilent]");
	...
	// Returns an array containing image class series ["dojoxRatingStarChecked", "dojoxRatingStarChecked", "", ""...].
	// Used in read only star creation. 
	getRepeatArray: function(config) {
		...
		for(i=0;i<count; i++) {
			result.push(i<rating?"dojoxRatingStarChecked":"")
		}
		return result;
	},
	...
	createNewInteraction: function(config) {
		try {
			...
			db=session.getDatabase(database.getServer(), interactionDB, false);
	...
	getComments: function(config) {
		try {
			...
			db=sessionAsSigner.getDatabase(database.getServer(),interactionDB,false);
	...
	getFavoriteDetail: function(config){
		try {
			...
			var favList=@DbLookup([database.getServer(),interactionDB], "FavoritesById", iId, 2, "[FailSilent]");
	...
	prepareMyFavorites: function(config, forceUpdate) {
		try {			
			...
			if(userKey!="") {
				result=@DbLookup([database.getServer(),interactionDB], "FavoritesByUserKey", userKey, 3, "[FailSilent]");
	...
	_errHandler:function(ccId, msg, displayUImsg, uiMsg) {
		print(view.getPageName()+"-xInvolve ("+ccId+") Error: "+msg);
		if(displayUImsg) {
